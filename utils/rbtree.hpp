...
class rbtree {
   ...
   private:  // Functions section
    ...
    link_type create_node(link_type childs = NULL, link_type parent = NULL,
                          pointer value = NULL);
   ...
   public:  // Member funcs
    ...
    void erase(iterator position) {
        if (position.node() == _header) return;
    ...
    const_iterator_pair equal_range(const value_type& val) const {
        return (ft::make_pair(lower_bound(val), upper_bound(val)));
    }
    iterator_pair equal_range(const value_type& val) {
        return (ft::make_pair(lower_bound(val), upper_bound(val)));
    }
...
rbtree<T, Compare, Alloc, Node_alloc>::rbtree(const key_compare& comp,
                                              const allocator_type& valloc,
                                              const node_allocator_type& nalloc)
    : _comp(comp), _valloc(valloc), _nalloc(nalloc), _size(0) {
    _nil = create_node();
    _root = _nil;
...
template <class T, class Compare, class Alloc, class Node_alloc>
template <class InputIterator>
rbtree<T, Compare, Alloc, Node_alloc>::rbtree(
...
template <class T, class Compare, class Alloc, class Node_alloc>
rbtree<T, Compare, Alloc, Node_alloc>::rbtree(const rbtree& copy)
    : _comp(copy._comp),
      _valloc(copy._valloc),
      _nalloc(copy._nalloc),
      _size(0) {
    _nil = create_node();
    _root = _nil;
    _header = create_node(_root, NULL, create_value(value_type()));
...
//  Helper Functions =============================
template <class T, class Compare, class Alloc, class Node_alloc>
bool rbtree<T, Compare, Alloc, Node_alloc>::_is_red(Node* nd) {
    return (color(nd) == true);
}

template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::pair_iterator_bool
rbtree<T, Compare, Alloc, Node_alloc>::insert(const_reference val) {
    ...
    if (searched != _nil) return (ft::make_pair(searched, false));
    return (ft::make_pair(_insert(curr_tmp, p_tmp, val), true));
}

template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::iterator
rbtree<T, Compare, Alloc, Node_alloc>::insert(iterator position,
                                              const value_type& val) {
    iterator ret;
    iterator searched = search(val);

    if (searched != _nil) return (searched);
    if (position == begin()) {
...
template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::iterator
rbtree<T, Compare, Alloc, Node_alloc>::_insert(link_type insert_to,
                                               link_type parent,
                                               const_reference val) {
    link_type new_node;
...
template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::iterator
rbtree<T, Compare, Alloc, Node_alloc>::_insert_fixup(link_type& node) {
    link_type tmp;
    link_type node_tmp;
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_rotate_right(link_type node) {
    link_type tmp;

    tmp = left(node);
    left(node) = right(tmp);
    if (right(tmp) != _nil) parent(tmp->right) = node;
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_rotate_left(link_type node) {
    link_type tmp;

    tmp = right(node);
    right(node) = left(tmp);
    if (left(tmp) != _nil) parent(tmp->left) = node;
    parent(tmp) = parent(node);
    if (node == _root)
        _root = tmp;
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_swap_colors(link_type nd) {
    nd->color = !nd->color;
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_transplant(link_type u,
                                                        link_type v) {
    if (parent(u) == _header)
        _root = v;
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_delete_fixup(link_type node) {
    link_type brother;
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_clear_node(link_type node) {
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_delete_node(link_type node) {
    link_type y = node;
    ...
    if (y_color == false) _delete_fixup(x);
    if (_size != 1) {
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::show() {
    std::cout << "==========================================="
              << "\n";
    _show(_root, 0);
}
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_show(link_type nd, int place) {
    if (nd == _nil) {
        for (int i = 5; i < place + 5; i++) std::cout << " ";
    ...
    for (int i = 5; i < place; i++) std::cout << " ";

    if (color(nd) == 1) {
        std::cout << RED << nd->value->first << " " << nd->value->second
                  << RESET << "\n";
    } else {
        std::cout << YELLOW << nd->value->first << " " << nd->value->second
                  << RESET << "\n";
    }
...
template <class T, class Compare, class Alloc, class Node_alloc>
void rbtree<T, Compare, Alloc, Node_alloc>::_clear(Node* nd) {
    if (nd == _nil) return;
    _clear(left(nd));
    _clear(right(nd));
...
template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::link_type
rbtree<T, Compare, Alloc, Node_alloc>::minimum(link_type x) {
    while (left(x) != _nil) x = left(x);
    return (x);
}
template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::link_type
rbtree<T, Compare, Alloc, Node_alloc>::maximum(link_type x) {
    while (right(x) != _nil) x = right(x);
    return (x);
}
template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::pointer
rbtree<T, Compare, Alloc, Node_alloc>::create_value(const_reference val) {
    pointer value;
...
template <class T, class Compare, class Alloc, class Node_alloc>
typename rbtree<T, Compare, Alloc, Node_alloc>::link_type
rbtree<T, Compare, Alloc, Node_alloc>::create_node(link_type childs,
                                                   link_type parent,
                                                   pointer value) {
    link_type created;
